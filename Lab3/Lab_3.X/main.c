...
void __interrupt() ISR(void) {//Interrupciones
    ...
    if (PIR1bits.ADIF == 1) { //Termino de convevrtir?
        if (pot == 0) {//Chequea la bandera del ADC
            ...
            pot = 1; // La proxima conversion entrara al siguiente loop
            voltaje = (val_pot1 * 5.0) / 255; //Convierto de binario a float el 
            //el valor de mi adresh
            Lcd_Set_Cursor(2, 1);//indico que el voltaje vaya a donde corresponde
            sprintf(buffer, "%.2f", voltaje); //esta funcion toma mi float, y lo
            // convierte en un char de 2 decimales
            Lcd_Write_String(buffer); // Display del valor del voltaje
            
        }
        else if (pot == 1) {
            val_pot2 = ADRESH; //Copia el valor de la conversion al puerto C
            adc_fin = 0; //Apagar bandera de copiando conversion
            pot = 0; //La proxima conversion entra a un loop
            voltaje = (val_pot2 * 5.0) / 255; //conversion
            Lcd_Set_Cursor(2, 7); // Nueva posicion segundo voltaje
